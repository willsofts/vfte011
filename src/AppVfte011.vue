...
<template>
  ...
  <div class="pt-page pt-page-current pt-page-controller search-pager">
    <PageHeader ref="pageHeader" :labels="labels" pid="vfte011" version="1.0.0" showLanguage="true" @language-changed="changeLanguage" :multiLanguages="multiLanguages" :build="buildVersion" :visible="displayPageHeader" />
...
export default {
    ...
    this.$nextTick(async () => {
      ...
      startApplication("vfte011",(data) => {
        ...
        if(data.type=="language") {
          ...
          loadAndMergeLabel("vfte011", (success) => {
            if (success) {
  ...
  methods: {
    ...
    dataSelected(item,action) {
      ...
      if("edit"==action) {
        this.$refs.entryForm.retrieveRecord({reservenum: item.reservenum});
      } else if("delete"==action) {
        this.$refs.entryForm.startDeleteRecord({reservenum: item.reservenum});
      }
    },
    dataInsert(filters) {
      //listen action from search form
