    ...
    <template v-slot:default>
      <div class="row row-height">
        <div class="col-height col-md-5">
          <label for="reservenum">{{ labels.reservenum_label }}</label>
          <div class="input-group has-validation" :class="{'has-error': v$.reservenum.$error}">
            <InputMask ref="reservenum" v-model="localData.reservenum" id="reservenum" picture="(50)x" /> 
            <label class="required">*</label>
          </div>
          <span v-if="v$.reservenum.$error" class="has-error">{{ v$.reservenum.$errors[0].$message }}</span>
        </div>
      </div>
      <div class="row row-height">
        <div class="col-height col-md-9">
          <label for="remarks">{{ labels.remarks_label }}</label>
          <input ref="remarks" type="text" v-model="localData.remarks" id="remarks" class="form-control input-md" maxlength="100" />
        </div>
      </div>
    </template>
...
export default {
  ...
  setup(props) {
    ...
    const validateRules = computed(() => { 
      return {
        reservenum: { required: requiredMessage() },
      } 
    });
        ...
        $.ajax({
          ...
          success: (data) => {
            ...
            successbox(() => {
              ...
              this.resetRecord();
              setTimeout(() => { this.$refs.reservenum.focus(); },100);
